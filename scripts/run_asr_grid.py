"""Decode all utterances with all models in the asr_grid config.

Iterates over default_models + careful_models, calling run_model() for each.
Caches results under data/asr_cache/{model_slug}/. Models already cached
(e.g., small and large-v3 from Round 1) are skipped automatically.
"""
...
def main():
    parser = argparse.ArgumentParser(description="ASR grid decode (Round 3)")
    ...
    default_models = grid.get("default_models", [])
    careful_models = grid.get("careful_models", [])

    # Deduplicate: union of grid models + original asr models
    all_models = list(dict.fromkeys(
        default_models + careful_models
        + [cfg["asr"]["default_model"], cfg["asr"]["careful_model"]]
    ))

    print(f"ASR grid: {len(all_models)} models")
    for m in all_models:
        print(f"  - {m}")
    ...
    # Run each model sequentially (never co-load multiple large models)
